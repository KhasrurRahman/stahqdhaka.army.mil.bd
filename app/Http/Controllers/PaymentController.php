...
class PaymentController extends Controller
{
    ...
    public function paymentReportData(Request $request)
    {
        ...
        $total_amount= Payment::sum('credit');
       
        
      return view('payment_gateway.payment-report-list', compact('total_amount','sticker_categories','applicant_name','applicant_type','sticker_type','inspec_from_date','inspec_to_date','ba','rank','reg_no','phone','vehicle_type'));
    ...
    public function paymentReportDatatable(Request $request)
    {
        ...
        , 'applications.sticker_category','applications.glass_type', 'vehicle_types.name as vehicle_name','applicant_details.applicant_BA_no','applicant_details.address','ranks.name as rank_name','vehicle_stickers.reg_number','payments.*')->get();
        ...
        return DataTables::of($query)
            ...
            ->addColumn('reg_number', function ($query) {
                $reg_number = ($query->reg_number) ? $query->reg_number : '';
                return $reg_number;
            })
            ...
            ->rawColumns(['name', 'type', 'sticker_category', 'create_at', 'created_by', 'credit','vehicle_name','glass_type','phone','rank_name','applicant_BA_no','reg_number','address'])
            ->toJson();
    }
    public function resenderMassageList(){
        return view('payment_gateway.resender_paid_list');
       
    }
    public function resenderMassageDatatable(){

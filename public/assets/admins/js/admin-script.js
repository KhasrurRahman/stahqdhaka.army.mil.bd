...
$(document).ready(function () {
  ...
  var file = "";
  //start free approval//
  $(document).on("change", ".free_sticker_ckbox", function () {
    if (this.checked) {
      $(".free_sticker_expand").show();
    } else {
      $(".free_sticker_expand").hide();
    }
  });
  //end free approval//
  ...
  var other_sms = new Array();
  var other_sms_key = "";
  $(document).on("change", ".other_sms", function () {
    $(".other_sms").not(this).prop("checked", false);
    if (this.checked) {
      other_sms_key = $(this).val();
      //other_sms.push(other_sms_key);
      other_sms = $(this).val();
    } else {
      other_sms.pop(other_sms_key);
    }
  });
  var rejectSmsType = "";
  ...
  $("#approve_App").on("click", function (e) {
    e.preventDefault();
    var form = $("#approve_App_frm")[0];
              ...
              .done(function (response) {
                ...
                if (response[1] == "success") {
                  ...
                  swal("Approved!", response[0], "success").then(function () {
                    $("#app_status").replaceWith(
                      "<span id='app_status' style='color: red'>" +
                        response[2]["app_status"] +
                        "</span>"
                    );
                    $("[data-dismiss=modal]").trigger({ type: "click" });
                    $("div.app_status_wrapper").after(
                      '<div class="col-md-3 sticker_type_wrapper"><span>Sticker Type</span></div><div class="col-md-9"><span>' +
                        response[2]["sticker_category"] +
                        "</span></div>" +
                        '<div class="col-md-3"><span>Delivery Date</span></div><div class="col-md-9"><span>' +
                        response[4] +
                        "</span></div>"
                    );
                    $(".follow-up-table tbody")
                      ...
                      .prepend(
                        "<td>#</td><td>" +
                          response[5]["created_at"] +
                          "</td><td>" +
                          response[5]["status"] +
                          "</td><td>" +
                          response[5]["comment"] +
                          "</td><td>on process</td><td>" +
                          response[5]["updated_by"] +
                          "</td>"
                      );
                    location.reload();
                  });
                  ...
                  swal("Not This Date!", response[1], "warning").then(
                    function () {
                      $("[data-dismiss=modal]").trigger({ type: "click" });
                    }
                  );
  ...
  $("#reject_App").on("click", function () {
    ...
    if (missMatch.length > 0 || custom_sms_text != "" || other_sms.length > 0) {
      swal({
        title: "Are you sure?",
                ...
                if (response[1] == "success") {
                  ...
                  swal("Rejected!", response[0], "success").then(function () {
                    ...
                    $(".follow-up-table tbody")
                      ...
                      .prepend(
                        "<td>#</td><td>" +
                          response[4]["created_at"] +
                          "</td><td>" +
                          response[4]["status"] +
                          "</td><td>" +
                          response[4]["comment"] +
                          "</td><td>on process</td><td>" +
                          response[4]["updated_by"] +
                          "</td>"
                      );
                  });
                } else if (response[1] == "fail") {
      ...
      if (rejectSmsType == "custom_case") {
        swal(
          "Oops...",
          "Please Select any SMS Template for which application will be rejected!",
          "error"
        );
      } else {
        swal(
          "Oops...",
          "Please Tick any for which application will be rejected!",
          "error"
        );
      }
    }
  ...
  // Hold Application
  $("#confirm-hold").on("click", function () {
    var app_id = $(this).data("number");
    var holdingMessage = $("#holdMessage").val();
    $.ajaxSetup({
      headers: {
    ...
    swal({
      title: "Are you sure?",
      text: "You want to hold this application!",
      type: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
        ...
        return new Promise(function (resolve) {
          $.ajax({
            ...
            success: function (resultData) {
              swal(
                "Application on Hold",
                "Application moved to hold list.",
                "success"
              ).then(function () {
                location.reload();
              });
          ...
          $.ajax({
            ...
            .done(function (response) {
              if (response[0] == "success") {
                swal(
                  "Done!",
                  "Your requested SMS has been sent successfully",
                  "success"
                ).then(function () {
                  $("span.sms-failed").remove();
                  $(".sms-status-replace").replaceWith(
              ...
              if (response[0] == "fail") {
                swal(
                  "Sorry!",
                  "Your requested SMS has not sent",
                  "warning"
                ).then(function () {});
              }
            })
  ...
  $("input#discount_amount").on(
    "keyup bind input cut copy paste change",
    function () {
      var discount = 0;
      var total = 0;
      var amount = 0;
          ...
          $.ajax({
            ...
            .done(function (response) {
              if (response[1] == "failed") {
                ...
                swal("Congratulation!", response[0], "success").then(
                  function () {
                    $("[data-dismiss=modal]").trigger({ type: "click" });
                    $(".follow-up-table tbody")
                      .prepend("<tr />")
                      ...
                      .prepend(
                        "<td>#</td><td>" +
                          response[3]["created_at"] +
                          "</td><td>" +
                          response[3]["status"] +
                          "</td><td>" +
                          response[3]["comment"] +
                          "</td><td>on process</td><td>" +
                          response[3]["updated_by"] +
                          "</td>"
                      );
                  }
                );
      ...
      success: function (response) {
        ...
        var rowNode = (table.row
          .add([
            ...
            response[1]["creator"],
            response[1]["updater"],
            '<button class="btn btn-info view-sms mr-1" data-template="' +
              response[1]["sms_template_name"] +
              '" data-subject="' +
              response[1]["sms_subject"] +
              '" data-message="' +
              ...
              '" data-id="' +
              response[1]["id"] +
              '" data-toggle="modal" data-target="#view_template_modal"><i class="fas fa-eye"></i></button>' +
              '<button class="btn btn-info edit-sms mr-1" data-template="' +
              response[1]["sms_template_name"] +
              '" data-subject="' +
              response[1]["sms_subject"] +
              '" data-message="' +
              ...
              '<button class="btn btn-danger delete-sms ml-0" data-id="' +
              response[1]["id"] +
        ...
        if (response[1]["id"] == id) {
          var rData = [
            ...
            '<button class="btn btn-info view-sms mr-1" data-template="' +
              ...
              '" data-id="' +
              response[1]["id"] +
              '" data-toggle="modal" data-target="#view_template_modal"><i class="fas fa-eye"></i></button><button class="btn btn-info edit-sms mr-1" data-template="' +
              response[1]["sms_template_name"] +
              '" data-subject="' +
              response[1]["sms_subject"] +
              '" data-message="' +
              ...
              '" data-id="' +
              response[1]["id"] +
              '" data-toggle="modal" data-target="#edit_template_modal"><i class="fas fa-edit"></i></button><button class="btn btn-danger delete-sms ml-0" data-id="' +
              response[1]["id"] +
              '" data-toggle="modal" data-target="#delete_template_modal"><i class="fas fa-trash-alt"></i></button>',
          ];
          table
  ...
  function printSMSErrorMsg(msg) {
    ...
    $.each(msg, function (key, value) {
      $(".print-error-msg")
        .find("ul")
        .append(
          '<li> <i class="fas fa-exclamation-triangle"></i> ' + value + "</li>"
        );
    });
  }
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        var rowNode = table.row
          .add([
            '<b class="serial">' + idx + "</b>",
            response[1]["depositor_name"],
            response[1]["bank_name"],
            response[1]["bank_acc_no"],
            response[1]["deposit_date"],
      ...
      success: function (response) {
        ...
        var rowNode = (table.row
          .add([
            ...
            '<button class="btn btn-info edit-admin mr-1" data-name="' +
              response[1]["name"] +
              '" data-email="' +
              response[1]["email"] +
              '" data-role="' +
              ...
              response[1]["id"] +
              '" data-toggle="modal" data-target="#edit_template_modal"><i class="fas fa-edit"></i></button>' +
              '<button class="btn btn-danger delete-admin ml-0" data-id="' +
              response[1]["id"] +
              '" data-toggle="modal" data-target="#delete_template_modal"><i class="fas fa-trash-alt"></i></button>',
          ])
          .order([0, "dsc"])
        ...
        if (response[1]["id"] == id) {
          var row_data = [
            ...
            '<button class="btn btn-info edit-admin mr-1" data-name="' +
              ...
              '" data-id="' +
              response[1]["id"] +
              '" data-toggle="modal" data-target="#edit_template_modal"><i class="fas fa-edit"></i></button><button class="btn btn-danger delete-admin ml-0" data-id="' +
              response[1]["id"] +
              '" data-toggle="modal" data-target="#delete_template_modal"><i class="fas fa-trash-alt"></i></button>',
          ];
          table
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response[1]["id"] == id) {
          ...
          $("#" + tr_id)
            .find("td:eq(2)")
            .html(response[1]["user_name"]);
          $("#" + tr_id)
            .find("td:eq(3)")
            .html(response[1]["email"]);
          $("#" + tr_id)
            .find("td:eq(4)")
            .html(response[1]["phone"]);
          $("#" + tr_id)
            .find("td:eq(5)")
            .html(response[1]["role"]);
          $("#" + tr_id)
            .find("td:eq(6)")
            .html(
              '<button class="btn btn-info edit-client" data-name="' +
                response[1]["name"] +
                '" data-email="' +
                response[1]["email"] +
                '" data-role="' +
                ...
                '" data-id="' +
                response[1]["id"] +
                '" data-toggle="modal" data-target="#edit_client_modal"><i class="fas fa-edit"></i></button>'
            );
          swal("Congratulation!", response[0], "success").then(function () {
      ...
      success: function (response) {
        ...
        var rowNode = (table.row
          .add([
            ...
            '<button class="btn btn-info edit-sticker mr-1" data-name="' +
              response[1]["name"] +
              '" data-price="' +
              response[1]["price"] +
              '" data-duration="' +
              ...
              response[1]["id"] +
              '" data-toggle="modal" data-target="#edit_template_modal"><i class="fas fa-edit"></i></button>' +
              '<button class="btn btn-danger delete-sticker ml-0" data-id="' +
              response[1]["id"] +
              '" data-toggle="modal" data-target="#delete_template_modal"><i class="fas fa-trash-alt"></i></button>',
          ])
          .order([0, "dsc"])
        ...
        if (response[1]["id"] == id) {
          var row_data = [
            ...
            '<button class="btn btn-info edit-sticker mr-1" data-name="' +
              ...
              '" data-id="' +
              response[1]["id"] +
              '" data-toggle="modal" data-target="#edit_template_modal"><i class="fas fa-edit"></i></button><button class="btn btn-danger delete-sticker ml-0" data-id="' +
              response[1]["id"] +
              '" data-toggle="modal" data-target="#delete_template_modal"><i class="fas fa-trash-alt"></i></button>',
          ];
          table

...
<div class="content-area" id="app-content-area" style="position: relative;">
  <div id="content-heading">
    <h4>Cantonment User's Vehicle Sticker
      ...
      Renew
      @endif
    Application Form for Defence</h4>
  </div>
  ...
  <div id="" class="Applycationform">
   <div class="tab-content" id="myTabContent">
    <div class="tab-pane show fade active" id="app-info" role="tabpanel" aria-labelledby="app-info-tab">
      <div class="container-fluid">
        <form id="applicant_detail_form-DEF" class="form Applycationform" enctype="multipart/form-data">
          ...
          <div class="row">
            ...
            <div class="col-md-8">
              ...
              <button type="button" name="" class="btn btn-info change-btn" data-photo="{{!empty(auth()->guard('applicant')->user()->applicantDetail->applicant_photo)?url(auth()->guard('applicant')->user()->applicantDetail->applicant_photo):url($runningApp->applicant->applicantDetail->applicant_photo)}}">Change </button>
            ...
            @if(!empty(auth()->guard('applicant')->user()->applicantDetail->is_spouseOrChild) && auth()->guard('applicant')->user()->applicantDetail->is_spouseOrChild==true)
            <div class="col-md-2 offset-md-1">
            ...
            <div class="col-md-8 applicant_spouse_child">
              @if(!empty(auth()->guard('applicant')->user()->applicantDetail->spouseOrParent_BA_no))
            ...
            <div class="col-md-8 applicant_spouse_child mt-2">
             @if(!empty(auth()->guard('applicant')->user()->applicantDetail->spouseOrParent_Rank_id))
             <select name="spouse_parents_rank" id="spouse_parents_rank" class="form-control in-form mandatory" >
              <option  selected value="{{auth()->guard('applicant')->user()->applicantDetail->spouseOrParent_Rank_id}}">{{auth()->guard('applicant')->user()->applicantDetail->spouseOrParentRank->name}}</option>
          ...
          <div class="col-md-8 applicant_spouse_child ">
            @if(!empty(auth()->guard('applicant')->user()->applicantDetail->spouseOrParent_Name))
          ...
          <div class="col-md-8 applicant_spouse_child mt-2">
           @if(!empty(auth()->guard('applicant')->user()->applicantDetail->spouse_parents_units_id))

           <select name="spouse_parents_unit" id="spouse_parents_unit" class="form-control in-form mandatory" >
            <option  selected value="{{auth()->guard('applicant')->user()->applicantDetail->spouse_parents_units_id}}" >{{auth()->guard('applicant')->user()->applicantDetail->spouseParentUnit->name}}</option>
        ...
        <div class="col-md-8 not_applicant_spouse_child">
          @if(!empty(auth()->guard('applicant')->user()->applicantDetail->applicant_BA_no))
        ...
        <div class="col-md-8 mt-2 not_applicant_spouse_child" >
          @if(!empty(auth()->guard('applicant')->user()->applicantDetail->rank_id))
          <select name="applicant_rank" id="applicant_rank" class="form-control in-form mandatory" >
           
            @if(isset($ranks) && count($ranks)>0)
            @foreach($ranks as $rank)
            <option value="{{$rank->id}}"  @if(auth()->guard('applicant')->user()->applicantDetail->rank_id==$rank->id)selected @endif>{{$rank->name}}</option>
            @endforeach
            @endif
          </select>
          @else
        ...
        <div class="col-md-8 not_applicant_spouse_child">
          @if(!empty(auth()->guard('applicant')->user()->applicantDetail->is_applicant_retired))
    ...
    @if(is_file($slash) || !empty($runningApp->applicant->applicantDetail->nid_photo))
      ...
      @endif
      <div class="col-md-2 offset-md-1">
        <label for="" class="label-form">Def Id card/Rtd ID/Cro Book Copy</label> <span>*</span> <br>
        <small></small>
      </div>
      <div class="col-md-8">
        @if(!empty(auth()->guard('applicant')->user()->applicantDetail->defIdCopy) || !empty($runningApp->applicant->applicantDetail->defIdCopy))
        <div>
      ...
      <div class="col-md-2 offset-md-5">
        <button type="submit" class="btn btn-primary custm-btn" id="def-E-n-btn1">Save & Continue</button>
      </div>
    </div>
  </form>
</div>
...
<div class="col-md-2 offset-md-1" id="Necessity-div">
  <label for="" class="label-form">Necessity of using Cantonment gate</label> <span></span> <br>
  <small></small>
</div>
...
@if( (!empty($allocated_sticker->application->vehicleinfo->glass_type) && $allocated_sticker->application->vehicleinfo->glass_type!= "transparent") || (!empty($runningApp->vehicleinfo->glass_type)&& $runningApp->vehicleinfo->glass_type!="transparent"))

<div class="col-md-2 offset-md-1" >
  <label for="" class="label-form">Type of Glass(if not transparent)</label> <span></span> <br>
  <small></small>
</div>
<div class="col-md-8">
...
<div class="col-md-8 is_transparent_hide" >
  <select name="glass_type" id="glass_type" class="form-control in-form mandatory" >
    ...
    <option  selected value="{{!empty($allocated_sticker->application->vehicleinfo->glass_type)?$allocated_sticker->application->vehicleinfo->glass_type:$runningApp->vehicleinfo->glass_type}}">{{!empty($allocated_sticker->application->vehicleinfo->glass_type)?$allocated_sticker->application->vehicleinfo->glass_type:$runningApp->vehicleinfo->glass_type}}</option>
...
<div class="tab-pane " id="driver" role="tabpanel" aria-labelledby="driver-tab">
  <div class="container-fluid">
    ...
    <div class="col-md-8 not_self_driven" hidden>
      ...
      @if( (isset($driver_address['permanent']) !='') && (isset($driver_address['present']['flat']) == isset($driver_address['permanent']['p_flat'])) && (isset($driver_address['present']['house']) == isset($driver_address['permanent']['p_house'])) && (isset($driver_address['present']['road']) == isset($driver_address['permanent']['p_road'])) && (isset($driver_address['present']['block'])== isset($driver_address['permanent']['p_block'])) && (isset($driver_address['present']['area']) == isset($driver_address['permanent']['p_area'])))
    ...
    <div class="col-md-1">
      <button type="submit"  class="btn btn-primary  custm-btn" id="E-n-btn3" >Save & Continue</button>
    </div>
  </div>
</form>
</div>

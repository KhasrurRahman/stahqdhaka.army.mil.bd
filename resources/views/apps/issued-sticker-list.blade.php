    ...
    <div class="content-area" style="padding-top: 15px;">
        <div class="container-fluid pl-0 pr-0">
            ...
            <div class="row col-md-12" style="text-align: center">
                <b>All Issued Sticker List</b> <br>
            </div>
            <div class="panel-heading">
                <div class="invoice_date_filter" style="">
                    <form action="{{url('/all-issued/application')}}" method="GET">
                        {{csrf_field()}}
                        <table>
                            <tr>
                                <td>
            ...
            <div class="panel-body" style="padding:10px 0;">
                <div id="example-wrapper-2" class="table-responsive">
                    ...
                    <table id="table" class="table table-bordered dt-responsive" style="text-align: center; width:100%;">
                        <thead>
                        <tr>
                            <th scope="col">#</th>
                        ...
                        <tbody>
                            {{-- @if(isset($apps))
                                ...
                                @foreach($apps as $key => $app)
                                    @php
                                        $def = App\application::where(['app_status'=>'issued','type'=>'def','sticker_category'=>$app->sticker_category])->count();
                                        $non_def =  App\application::where(['app_status'=>'issued','type'=>'non-def','sticker_category'=>$app->sticker_category])->count();
                                        $transparent = App\application::where(['app_status'=>'issued','glass_type'=>'transparent','sticker_category'=>$app->sticker_category])->count();
                                        $not_transparent = App\application::where(['app_status'=>'issued','sticker_category'=>$app->sticker_category])->where('glass_type','!=','transparent')->count();
                                        $ps_issued = App\application::where(['app_status'=>'issued','ps_approved'=>1,'sticker_category'=>$app->sticker_category])->count();
                                        $total_def += $def;
                                        $total_non_def += $non_def;
                                        $total_transparent += $transparent;
                                        $total_not_transparent += $not_transparent;
                                        $total_ps_issued += $ps_issued;
                                    @endphp
                                    <tr>
                            ...
                            @if(isset($apps))
                                ...
                                @foreach ($stickerTypes as $key => $sticker)
                                        ...
                                        @php
                                            // $query = App\application::where(['app_status'=>'approved','sticker_category'=>$sticker->name]);
                                            
                                            if($inspec_from_date !='' && $inspec_to_date !=''){
												                          $frmDt	= date('Y-m-d', strtotime($inspec_from_date));
												                          $toDt	= date('Y-m-d', strtotime($inspec_to_date));
                                                //$applicaation_ids = App\VehicleSticker::whereBetween('issue_date', [date('Y-m-d', strtotime($inspec_from_date)), date('Y-m-d', strtotime($inspec_to_date))])->pluck('application_id');
                                                // $query->whereBetween('app_date', [date('Y-m-d', strtotime($inspec_from_date)), date('Y-m-d', strtotime($inspec_to_date))]);
                                                if ($sticker_category) {
													
                                                    $def = App\application::whereIn('app_status', ['issued','warning','expired'])
													->where('sticker_category',$sticker->name)
													->where(['type'=>'def'])
													->whereRaw("id IN ( SELECT application_id FROM vehicle_stickers WHERE issue_date BETWEEN '$frmDt' AND '$toDt')")
													->where('sticker_category', $sticker_category)
													->count();
													
                                                    $non_def =  App\application::whereIn('app_status', ['issued','warning','expired'])
													->where(['sticker_category'=>$sticker->name])
													->where(['type'=>'non-def'])
												
													->whereRaw("id IN ( SELECT application_id FROM vehicle_stickers WHERE issue_date BETWEEN '$frmDt' AND '$toDt')")
													->where('sticker_category', $sticker_category)->count();
													
                                                    $transparent = App\application::whereIn('app_status', ['issued','warning','expired'])
													->where(['sticker_category'=>$sticker->name])
													->where(['glass_type'=>'transparent'])
													->whereRaw("id IN ( SELECT application_id FROM vehicle_stickers WHERE issue_date BETWEEN '$frmDt' AND '$toDt')")
													->where('sticker_category', $sticker_category)->count();
													
                                                    $not_transparent = App\application::whereIn('app_status', ['issued','warning','expired'])
													->where(['sticker_category'=>$sticker->name])
													->where('glass_type','!=','transparent')
													->whereRaw("id IN ( SELECT application_id FROM vehicle_stickers WHERE issue_date BETWEEN '$frmDt' AND '$toDt')")
													->where('sticker_category', $sticker_category)->count();
													
                                                    $ps_issued = App\application::whereIn('app_status', ['issued','warning','expired'])
													->where(['sticker_category'=>$sticker->name])
													->where(['ps_approved'=>1])
													->whereRaw("id IN ( SELECT application_id FROM vehicle_stickers WHERE issue_date BETWEEN '$frmDt' AND '$toDt')")
													->where('sticker_category', $sticker_category)->count();
                                                    // new add
                                                    $free_approved = App\application::whereIn('app_status', ['issued'])->where(['sticker_category'=>$sticker->name])->where(['sticker_category'=>$sticker_category])->where('issue_type', 'free')->whereRaw("id IN (SELECT application_id FROM application_notifies WHERE sticker_delivery_date BETWEEN '$frmDt' AND '$toDt')")->count();
													
													
                                                }else {
                                                    $def = App\application::whereIn('app_status', ['issued','warning','expired'])->where('sticker_category',$sticker->name)->where(['type'=>'def'])->where(['type'=>'def'])->whereRaw("id IN ( SELECT application_id FROM vehicle_stickers WHERE issue_date BETWEEN '$frmDt' AND '$toDt')")->count();
                                                    $non_def =  App\application::whereIn('app_status', ['issued','warning','expired'])->where(['sticker_category'=>$sticker->name])->where(['type'=>'non-def'])->whereRaw("id IN ( SELECT application_id FROM vehicle_stickers WHERE issue_date BETWEEN '$frmDt' AND '$toDt')")->count();
                                                    $transparent = App\application::whereIn('app_status', ['issued','warning','expired'])->where(['sticker_category'=>$sticker->name])->where(['glass_type'=>'transparent'])->whereRaw("id IN ( SELECT application_id FROM vehicle_stickers WHERE issue_date BETWEEN '$frmDt' AND '$toDt')")->count();
                                                    $not_transparent = App\application::whereIn('app_status', ['issued','warning','expired'])->where(['sticker_category'=>$sticker->name])->where('glass_type','!=','transparent')->whereRaw("id IN ( SELECT application_id FROM vehicle_stickers WHERE issue_date BETWEEN '$frmDt' AND '$toDt')")->count();
                                                    $ps_issued = App\application::whereIn('app_status', ['issued','warning','expired'])->where(['sticker_category'=>$sticker->name])->where(['ps_approved'=>1])->whereRaw("id IN ( SELECT application_id FROM vehicle_stickers WHERE issue_date BETWEEN '$frmDt' AND '$toDt')")->count();
                                                    // new add
                                                    $free_approved = App\application::whereIn('app_status', ['issued'])->where(['sticker_category'=>$sticker->name])->where('issue_type', 'free')->whereRaw("id IN (SELECT application_id FROM application_notifies WHERE sticker_delivery_date BETWEEN '$frmDt' AND '$toDt')")->count();
                                                }
                                                
                                                
                                            }else {
                                                $def = App\application::whereIn('app_status', ['issued','warning','expired'])->where(['sticker_category'=>$sticker->name])->where(['type'=>'def','sticker_category'=>$sticker->name])->count();
                                                $non_def =  App\application::whereIn('app_status', ['issued','warning','expired'])->where(['sticker_category'=>$sticker->name])->where(['type'=>'non-def','sticker_category'=>$sticker->name])->count();
                                                $transparent = App\application::whereIn('app_status', ['issued','warning','expired'])->where(['sticker_category'=>$sticker->name])->where(['glass_type'=>'transparent','sticker_category'=>$sticker->name])->count();
                                                $not_transparent = App\application::whereIn('app_status', ['issued','warning','expired'])->where(['sticker_category'=>$sticker->name])->where(['sticker_category'=>$sticker->name])->where('glass_type','!=','transparent')->count();
                                                $ps_issued = App\application::whereIn('app_status', ['issued','warning','expired'])->where(['sticker_category'=>$sticker->name])->where(['ps_approved'=>1,'sticker_category'=>$sticker->name])->count();
                                                // new add
                                                $free_approved = App\application::whereIn('app_status', ['issued'])->where(['sticker_category'=>$sticker->name])->where(['sticker_category'=>$sticker->name])->where('issue_type', 'free')->count();
                                            }
...
@section('admin-script')
    ...
    <script>
        ...
        $(function() {
            $('#table').DataTable({
                ...
                iDisplayLength: 10,
                ajax: {
                    url: '{{ route('issued_apps.datatable') }}',
                    data: function (d) {
    ...
    <script type="text/javascript">
        function Print_approved_sticker_list(logo) { 
            ...
            mywindow.document.write('<div class="p-heading"><div class="logo"><img src="'+logo+'" style="width:90px;margin-left: 30px;margin-top: 15px; height:auto" /></div><div class="ptitle" ><h3>Station Headquarters Dhaka Cantonment</h3><p>Shaheed Sharani, Dhaka, Bangladesh</p><p>Contact: 01797-585010</p><h4>All Issued Applications</h4> <br></div><div class="qrcode"></div></div>');
        mywindow.document.write(document.getElementById('table').outerHTML); 

        mywindow.document.write('<div style="text-align:center;margin-top:15px;" ><h4 style="font-size:20px;margin:0;">Summary of All Issued Applications</h4></div>');  
